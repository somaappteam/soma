...
Future<void> main() async {
  ...
  appLogger.info('--- REALTIME & PROFILE CHECK ---');

  try {
    // 1. Check if 'profiles' has realtime enabled via a dummy query or checking publication
    final res = await client
        .rpc('get_table_realtime_status', params: {'t_name': 'profiles'});
    appLogger.info('Realtime status for profiles: $res');
  } catch (e) {
    appLogger.info('Could not check realtime via RPC: $e');
  }
  ...
  try {
    ...
    appLogger.info(
        'Recent profile IDs: ${users.map((final u) => u['id']).toList()}');
  } catch (e) {
    appLogger.info('Error fetching profiles: $e');
  }
}
