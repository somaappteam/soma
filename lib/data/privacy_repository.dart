...
class PrivacyRepository {
  ...
  Future<Map<String, dynamic>> exportUserDataSnapshot() async {
    ...
    snapshot['friendships'] = await _safeSelectList(
      table: 'friendships',
      filters: (final q) => q.or('requester_id.eq.$uid,addressee_id.eq.$uid'),
    );
    snapshot['message_requests'] = await _safeSelectList(
      table: 'message_requests',
      filters: (final q) => q.or('requester_id.eq.$uid,recipient_id.eq.$uid'),
    );
    ...
    snapshot['notifications'] = await _safeSelectList(
      table: 'notifications',
      filters: (final q) => q.eq('user_id', uid).order('created_at'),
      limit: 2000,
    );
    ...
    snapshot['chat_reports'] = await _safeSelectList(
      table: 'chat_reports',
      filters: (final q) => q.eq('reporter_id', uid).order('created_at'),
    );
