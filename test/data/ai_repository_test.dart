...
class _FakeInvoker implements EdgeFunctionInvoker {
  ...
  @override
  Future<FunctionResponse> invoke(String name, {Object? body}) => _handler(name, body);
}

FunctionResponse _response(dynamic data) => FunctionResponse(data: data, status: 200);

void main() {
  test('translateText returns expected value', () async {
    final repo = AiRepository(_FakeInvoker((_, __) async => _response({'text': 'Hola'})));
  ...
  test('translateText throws on invalid payload', () async {
    final repo = AiRepository(_FakeInvoker((_, __) async => _response({'value': 'missing'})));
  ...
  test('translateText enforces output max length', () async {
    final repo = AiRepository(
      _FakeInvoker((_, __) async => _response({'text': List.filled(1201, 'x').join()})),
    );
  ...
  test('analyzePronunciation throws on schema mismatch', () async {
    final repo = AiRepository(
      _FakeInvoker((_, __) async => _response({'score': 180, 'difficult_words': 'bad', 'tip': 1})),
    );
